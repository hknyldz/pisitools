...
from pisilinux.scenarioapi.pspec import Pspec
from pisilinux.scenarioapi.actions import Actions
from pisilinux.scenarioapi.constants import *
from pisilinux.scenarioapi.withops import *
...
class Package:
    ...
    def create_pisilinux(self):
        os.system("pisilinux build %s -O %s > /dev/null 2>&1" % (consts.pspec_path, consts.repo_path))
    ...
    def get_file_name(self):
        ...
        found = glob.glob(pkg + consts.glob_pisilinuxs)
        if not found:
            raise Exception(_("No pisilinux package: %s* found.") % pkg)
