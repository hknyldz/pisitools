...
from pisilinux.util import join_path, remove_prefix, uncompress

# ActionsAPI Modules
import pisilinux.actionsapi
import pisilinux.actionsapi.get as get
from pisilinux.actionsapi.pisilinuxtoolsfunctions import *
from pisilinux.actionsapi.shelltools import *

from pisilinux.actionsapi import error

def dobin(sourceFile, destinationDirectory = '/usr/bin'):
    '''insert a executable file into /bin or /usr/bin'''
    ''' example call: pisilinuxtools.dobin("bin/xloadimage", "/bin", "xload") '''
...
def doexe(sourceFile, destinationDirectory):
    ...
    ''' example call: pisilinuxtools.doexe("kde-3.4.sh", "/etc/X11/Sessions")'''
    executable_insinto(join_path(get.installDIR(), destinationDirectory), sourceFile)
...
def dosed(sources, findPattern, replacePattern = '', filePattern = '', deleteLine = False, level = -1):
    ...
    ''' example call: pisilinuxtools.dosed("/etc/passwd", "caglar", "cem")'''
    ''' example call: pisilinuxtools.dosed("/etc/passwd", "caglar")'''
    ''' example call: pisilinuxtools.dosed("/etc/pass*", "caglar")'''
    ''' example call: pisilinuxtools.dosed("Makefile", "(?m)^(HAVE_PAM=.*)no", r"\1yes")'''
    ''' example call: pisilinuxtools.dosed("./", "^(CFLAGS) =", r"\1 +=", "Makefile", level = 1)
        will change: ./Makefile and ./*/Makefile'''
    ''' example call: pisilinuxtools.dosed("./", "^\s*g_type_init\(\)", filePattern = ".*.c", deleteLine = True)
    ...
    backupExtension = ".pisilinux-backup"
    sourceFiles = []
...
def dosbin(sourceFile, destinationDirectory = '/usr/sbin'):
    ...
    ''' example call: pisilinuxtools.dobin("bin/xloadimage", "/sbin") '''
    executable_insinto(join_path(get.installDIR(), destinationDirectory), sourceFile)
