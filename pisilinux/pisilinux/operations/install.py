...
def install_pkg_files(package_URIs, reinstall = False):
    """install a number of pisilinux package files"""
    ...
    if not reinstall:
        for x in package_URIs:
            if not x.endswith(ctx.const.delta_package_suffix) and x.endswith(ctx.const.package_suffix):
                pkg_name, pkg_version = pisilinux.util.parse_package_name(os.path.basename(x))
    ...
    for x in package_URIs:
        try:
            package = pisilinux.package.Package(x)
            package.read()
