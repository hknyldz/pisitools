...
def file_corrupted(pfile):
    ...
    if os.path.islink(path):
        if pisilinux.util.sha1_data(pisilinux.util.read_link(path)) != pfile.hash:
            return True
    else:
        try:
            if pisilinux.util.sha1_file(path) != pfile.hash:
                return True
        except pisilinux.util.FilePermissionDeniedError as e:
            raise e
    return False
...
def check_config_files(package):
    config_files = pisilinux.db.installdb.InstallDB().get_config_files(package)
    return check_files(config_files, True)

def check_package_files(package):
    files = pisilinux.db.installdb.InstallDB().get_files(package).list
