...
import pisilinux.dependency
import pisilinux.db.itembyrepo
...
class PackageDB(lazydb.LazyDB):
    ...
    def init(self):
        ...
        self.pdb = pisilinux.db.itembyrepo.ItemByRepo(self.__package_nodes, compressed=True)
        self.rvdb = pisilinux.db.itembyrepo.ItemByRepo(self.__revdeps)
        self.odb = pisilinux.db.itembyrepo.ItemByRepo(self.__obsoletes)
        self.rpdb = pisilinux.db.itembyrepo.ItemByRepo(self.__replaces)
    ...
    def get_package_repo(self, name, repo=None):
        ...
        package = pisilinux.metadata.Package()
        package.parse(pkg)
        return package, repo
        ...
        for pkg_name in self.rpdb.get_list_item():
            ...
            if replaces_tag:
                for node in replaces_tag.tags("Package"):
                    r = pisilinux.relation.Relation()
                    # XXX Is there a better way to do this?
                    r.decode(node, [])
                    if pisilinux.replace.installed_package_replaced(r):
