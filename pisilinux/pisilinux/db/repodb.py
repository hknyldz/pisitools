...
class RepoError(pisilinux.Error):
    pass
...
class RepoOrder:
    ...
    def get_order(self):
        ...
        #FIXME: get media order from pisilinux.conf
...
class RepoDB(lazydb.LazyDB):
    ...
    def get_repo_doc(self, repo_name):
        ...
        if File.is_compressed(index_path) or repo.indexuri.is_remote_file():
            ...
            index_path = pisilinux.util.join_path(ctx.config.index_dir(),
                                             repo_name, index)
        ...
        if not os.path.exists(index_path):
            ...
            return piksemel.newDocument("pisilinux")
    ...
    def get_repo(self, repo):
        return Repo(pisilinux.uri.URI(self.get_repo_url(repo)))

    #FIXME: this method is a quick hack around repo_info.indexuri.get_uri()
    def get_repo_url(self, repo):
        urifile_path = pisilinux.util.join_path(ctx.config.index_dir(), repo, "uri")
        uri = open(urifile_path, "r").read()
        return uri.rstrip()

    def add_repo(self, name, repo_info, at = None):
        repo_path = pisilinux.util.join_path(ctx.config.index_dir(), name)
        os.makedirs(repo_path)
        urifile_path = pisilinux.util.join_path(ctx.config.index_dir(), name, "uri")
        open(urifile_path, "w").write(repo_info.indexuri.get_uri())
        self.repoorder.add(name, repo_info.indexuri.get_uri())

    def remove_repo(self, name):
        pisilinux.util.clean_dir(os.path.join(ctx.config.index_dir(), name))
