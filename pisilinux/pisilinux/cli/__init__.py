...
class CLI(pisilinux.ui.UI):
    ...
    def close(self):
        pisilinux.util.xterm_title_reset()
    ...
    def formatted_output(self, msg, verbose = False, noln = False, column=":"):
        ...
        term_height, term_width = pisilinux.util.get_terminal_size()
    ...
    def choose(self, msg, opts):
        msg = str(msg)
        prompt = msg + pisilinux.util.colorize(' (%s)' % "/".join(opts), 'red')
        while True:
    ...
    def display_progress(self, **ka):
        ...
        if ka['operation'] in ["removing", "rebuilding-db"]:
            ...
            totalsize = '%.1f %s' % pisilinux.util.human_readable_size(ka['total_size'])
        ...
        if ka['percent'] == 100:
            self.output(pisilinux.util.colorize(_(' [complete]\n'), 'gray'))

    def status(self, msg = None):
        if msg:
            msg = str(msg)
            self.output(pisilinux.util.colorize(msg + '\n', 'brightgreen'))
            pisilinux.util.xterm_title(msg)

    def notify(self, event, **keywords):
        if event == pisilinux.ui.installed:
            msg = _('Installed %s') % keywords['package'].name
        elif event == pisilinux.ui.removed:
            msg = _('Removed %s') % keywords['package'].name
        elif event == pisilinux.ui.upgraded:
            msg = _('Upgraded %s') % keywords['package'].name
        elif event == pisilinux.ui.configured:
            msg = _('Configured %s') % keywords['package'].name
        elif event == pisilinux.ui.extracting:
