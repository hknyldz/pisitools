...
""")
    ...
    def __init__(self, args):
        ...
        self.repodb = pisilinux.db.repodb.RepoDB()
    ...
    def warn_and_remove(self, message, repo):
        ...
        pisilinux.api.remove_repo(repo)
    ...
    def run(self):
        ...
        if len(self.args) == 2:
            ...
            pisilinux.api.add_repo(name, indexuri, ctx.get_option('at'))

            if not ctx.get_option('no_fetch'):
                try:
                    pisilinux.api.update_repo(name)
                except (pisilinux.Error, IOError):
