...
class autocommand(type):
    def __init__(cls, name, bases, dict):
        ...
        if name is None:
            raise pisilinux.cli.Error(_('Command lacks name'))
        longname, shortname = name
        def add_cmd(cmd):
            if cmd in Command.cmd_dict:
                raise pisilinux.cli.Error(_('Duplicate command %s') % cmd)
            else:
...
class Command(object):
    ...
    def commands_string():
        ...
        for name in l:
            ...
            trans = gettext.translation('pisilinux', fallback=True)
    ...
    def get_command(cmd, fail=False, args=None):
        ...
        if fail:
            raise pisilinux.cli.Error(_("Unrecognized command: %s") % cmd)
        else:
            return None
    ...
    def __init__(self, args = None):
        ...
        self.parser = optparse.OptionParser(usage=getattr(self, "__doc__"),
                                            version="%prog " + pisilinux.__version__,
                                            formatter=pisilinuxHelpFormatter())
        self.options()
        self.commonopts()
    ...
    def commonopts(self):
        ...
        group.add_option("-D", "--destdir", action="store", default = None,
                     help = _("Change the system root for pisilinux commands"))
        ...
        group.add_option("-N", "--no-color", action="store_true", default=False,
                     help = _("Suppresses all coloring of pisilinux's output"))
    ...
    def process_opts(self):
        ...
        if self.options.destdir:
            ...
            if not os.path.exists(d):
                pisilinux.cli.printu(_('Destination directory %s does not exist. Creating directory.\n') % d)
                os.makedirs(d)
    ...
    def init(self, database = True, write = True):
        """initialize pisilinux components"""

        if self.options:
            ui = pisilinux.cli.CLI(self.options.debug, self.options.verbose)
        else:
            ui = pisilinux.cli.CLI()

        if (write and not os.access(pisilinux.context.config.packages_dir(), os.W_OK) or \
            ('sf' in self.get_name() and not os.access(os.path.join(ctx.config.info_dir(), ctx.const.files_ldb), os.W_OK))):
            raise pisilinux.cli.Error(_("You have to be root for this operation."))

        pisilinux.api.set_userinterface(ui)
        pisilinux.api.set_options(self.options)
        pisilinux.api.set_comar(self.comar and not ctx.get_option('ignore_comar'))
...
class pisilinuxHelpFormatter(optparse.HelpFormatter):
    def __init__(self,
                 indent_increment=1,
